...
inline int process_channel_dma(proxy_state_t *state, proxy_channel_t *ch, int *is_processed,
    ...
    // issue transport DMA
    {
        ...
        verb.is_stream = 0;
        verb.cstrm = NULL;
        /* Peek ahead: batch consecutive DMA ops via the optional host_ops.batch_hint hook.
         * Flush every NVSHMEM_PROXY_BATCH_FLUSH_INTERVAL ops (default 16). Transports that do not
         * install the hook pay zero cost.
         */
        struct nvshmem_transport *tcurr = state->transport[pe];
        if (tcurr->host_ops.batch_hint) {
            nvshmem_transport_batch_flag_t hint_flags = NVSHMEM_TRANSPORT_BATCH_FLAG_NONE;
            int flush_interval = nvshmemi_options.PROXY_BATCH_FLUSH_INTERVAL;
            if (j != nvshmemi_options.PROXY_REQUEST_BATCH_MAX - 1 &&
                ...
                if (next_flag_val == next_flag &&
                    ...
                    hint_flags = NVSHMEM_TRANSPORT_BATCH_FLAG_RMA;
                }
            }
            tcurr->host_ops.batch_hint(tcurr, NVSHMEMX_QP_DEFAULT, hint_flags);
        }
