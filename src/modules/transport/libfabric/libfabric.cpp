...
static int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
    ...
    if (is_write_comp) {
        ...
        map_key = (static_cast<uint64_t>(pe) << 32) |
        ...
        map_key = (static_cast<uint64_t>(pe) << 32) | sig_op->sequence_count;
    ...
    if (is_write_comp &&
        ...
        auto iter = signal_state.proxy_put_signal_comp_map.find(map_key);
...

    {
        ...
        while (true) {
            ...
            uint64_t key = (static_cast<uint64_t>(pe) << 32) | next_seq;
            ...
            if (it->second.type == NVSHMEMT_LIBFABRIC_COMP_ENTRY_SIGNAL) {
                ...
                do {
                    status =
                        libfabric_state->op_queue[ack_ep.domain_index]->getNextSends(&send_elem, 1);
                } while (try_again(transport, &status, &num_retries,
                                   NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDRCOPY_AMO_ACK,
                                   ack_ep.qp_index, progress_type::CompletionsOnly));

                if (status == 0) {
                    status = gdrcopy_amo_ack(transport, ack_ep, it->second.ack_entry.src_addr,
                                             next_seq, pe, &send_elem,
        ...
        if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
            ...
            do {
                status = fi_writemsg(ep.endpoint, &p_op_msg, FI_INJECT);
                               ...
                               NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_RMA_IMPL_OP_P_NON_EFA, ep.qp_index, progress_type::All));
        }
        ...
        do {
            if (imm_data) {
                ...
                status = fi_write(ep.endpoint, local->ptr, op_size, local_mr_desc, target_ep,
                           ...
                           NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_RMA_IMPL_OP_PUT, ep.qp_index, progress_type::All));
...
static int nvshmemt_libfabric_gdr_amo(struct nvshmem_transport *transport, int pe, void *curetptr,
    ...
    if (is_signal_only_op(verb.desc)) {
        ...
        status = get_next_seq_num_with_retry(transport, seq_counter, &sequence_count, qp_index,
                                             NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDR_AMO_GET_NEXT_SENDS);
        if (status) goto out;
        assert(sequence_count != NVSHMEM_STAGED_AMO_SEQ_NUM);
...
{
    state->host_signal_state.clear();
    state->proxy_signal_state.clear();
}
