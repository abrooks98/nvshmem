...
struct nvshmemt_libfabric_endpoint_seq_counter_t {
    ...
    /**
     * Frequency at which we send an ack for puts (without signal). For puts-only, we don't need the
     * ack for every message for semantic reasons. We only need need an occasional ack to handle
     * sequence number overflow correctly.
     */
    constexpr static uint32_t put_ack_freq = 64;

    /* Assert that index_mask is large enough to simplify some ranged ack return logic */
    ...
    /**
     * Mark a range of sequence numbers as complete, resulting from receiving a put ack. The
     * sequence range ends with end_seq.
     *
     * We send an ack for every put_ack_freq puts. Therefore, a put ack for <end_seq> is an
     * acknowledgement sequence numbers (end_seq - put_ack_freq + 1) to end_seq, inclusive. The
     * wraparound case is also handled.
     *
     * This code assumes the sequence range spans at most two categories. This will be true as long
     * as the index space is sufficiently larger than the put ack frequency, as static asserted
     * above.
     */
    void return_acked_seq_num_range_for_put(uint32_t end_seq) {
        assert(end_seq != NVSHMEM_STAGED_AMO_SEQ_NUM);

        uint32_t start_seq = (end_seq - put_ack_freq + 1) & sequence_mask;
